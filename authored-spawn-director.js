...
export function createAuthoredSpawnDirector(def={}){
  const raw=def?.spawnSets||{},clean=list=>(Array.isArray(list)?list:[]).filter(p=>Array.isArray(p)&&p.length>=2).map(p=>Object.freeze([finite(p[0]),finite(p[1]),finite(p[2],0)])),blue=Object.freeze(clean(raw.blue)),red=Object.freeze(clean(raw.red)),ffa=Object.freeze(clean(raw.ffa).length?clean(raw.ffa):[...blue,...red]);
