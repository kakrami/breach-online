...
// corner. The normal safety scorer chooses the least dangerous candidate on
// every life, which is essential for a fast 8-player close-quarters arena.
...
export function scoreSpawnCandidate({mode,team,x,y,z,actors=[],excludeId='',recentDeaths=[],recentSpawns=[],projectiles=[],throwables=[],now=Date.now(),lineOfSight,}){
  ...
  for(const actor of live){const d=distance2d({x,z},actor);minAny=Math.min(minAny,d);if(!enemyFor(mode,team,actor))continue;minEnemy=Math.min(minEnemy,d);if(d<19)nearEnemies++;if(d<=SPAWN_POLICY.lineOfSightDistance&&lineOfSight?.({x,y,z},actor))visibleEnemies++;if(d<=SPAWN_POLICY.facingThreatDistance&&facingThreat(actor,x,z)>=SPAWN_POLICY.facingThreatCos)facingEnemies++;}
  const safe=minEnemy>=SPAWN_POLICY.minEnemyDistance;let score=safe?1_000_000:0;
  score+=Math.min(minEnemy,120)*190+Math.min(minAny,45)*8;
  score-=visibleEnemies*62_000+facingEnemies*16_000+nearEnemies*8_500;
  score-=recentPenalty(recentDeaths,x,z,now,SPAWN_POLICY.recentDeathWindowMs,SPAWN_POLICY.recentDeathRadius,58_000);
  score-=recentPenalty(recentSpawns,x,z,now,SPAWN_POLICY.recentSpawnWindowMs,SPAWN_POLICY.recentSpawnRadius,21_000);
